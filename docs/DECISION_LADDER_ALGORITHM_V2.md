...
**Rule:**
`structure_level = swing_low` (Fallback to `ema_50_w`)
*(Implementation logic: The structural backbone is the most recent confirmed weekly higher-low. This serves as the anchor for the entire downside ladder.)*

### 3. Alert Level
**Definition:** The early warning boundary indicating momentum deceleration.
**Rule:**
`alert_level = structure_level * 1.05`
*(Implementation logic: A 5% buffer above the Structure level triggers the first Alert, indicating short-term trend exhaustion before structural damage occurs.)*

### 4. Quit Level
**Definition:** The absolute thesis invalidation point where the stock must be exited.
**Rule:**
`quit_level = structure_level * 0.95`
*(Implementation logic: A 5% confirmation buffer below the Structure level invalidates the uptrend thesis completely, avoiding fake-out wicks.)*
