...
This document is the **single source of truth** for all CAI Decision Ladder calculations. The backend engine must implement this specification exactly. Any modification to the Decision Ladder algorithm requires a new Algorithm ID and version.
...
**Rule:** 
The Add Level SHALL equal the breakout trigger generated by the MRI Breakout Engine.
*(Reference: MRI Breakout Engine / breakout_level. The Decision Ladder Engine must never independently calculate breakout levels).*

### 2. Structure Level (The Anchor)
**Definition:** The critical support boundary where the primary trend is severely threatened.
**Rule:**
`structure_level = primary_swing_low` (Fallback: `ema_50_w` -> `ema_20_w`)
*(Implementation logic: The structural backbone is the most recent confirmed PRIMARY weekly higher-low. This serves as the absolute anchor for the entire downside ladder.)*

### 3. Alert Level
**Definition:** The early warning boundary indicating momentum deceleration.
**Rule:**
`alert_level = structure_level + (1 * ATR_14_W)`
*(Implementation logic: A volatility-adjusted buffer (1 Average True Range) above the Structure level triggers the first Alert. This scales naturally with the stock's volatility instead of using an arbitrary percentage.)*

### 4. Quit Level
**Definition:** The absolute thesis invalidation point where the stock must be exited.
**Rule:**
`quit_level = structure_level * 0.98`
*(Implementation logic: Quit is fundamentally an EVENT, not just a price. It represents a confirmed weekly close below the Structure level with a 2% noise buffer to avoid fake-out wicks. If `current_price < quit_level`, the structural thesis is objectively broken.)*
...
* **Rule:** If `primary_swing_low` is NULL, the `structure_level` evaluates to the 50-week EMA (`ema_50_w`). If `ema_50_w` is also NULL, it evaluates to the 20-week EMA (`ema_20_w`). If all are NULL, `NOT_COMPUTED`. This guarantees the consistent fallback order: `Swing Low -> EMA50 -> EMA20 -> NOT_COMPUTED`.

### 3. Precedence in Disagreements (e.g., Swing Low > EMA50)
* **Rule:** Because the ladder now uses the **Derived Anchor Model**, mathematical inconsistencies (like Quit > Structure) are mathematically impossible. The system guarantees `Alert > Structure > Quit`.
...
* **Note:** This `decision_quality` is strictly internal database metadata and MUST NOT be leaked to the Phase 3 REST API.
