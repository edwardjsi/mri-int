...
// AAE × Management Integrity (Phase 5, 2026-06-17): verdict zone color
// mapping for the Digital Twin integrity panel. Mirrors GuidanceCheck
// / ConvictionEngine color conventions so the badge is instantly familiar.
                ...
                <div className="aae-results-grid">
                  {digitalTwinResult.error ? (
                    ...
                    <>
                      ...
                      {/* ── Management Integrity panel (AAE × Management Integrity, Phase 5) ── */}
                      {(() => {
                        const mi = digitalTwinResult.legacy_forensic?.layers?.management_integrity;
                        const graveyardRule = digitalTwinResult.legacy_forensic?.graveyard_rule;
                        const graveyardPenalty = digitalTwinResult.legacy_forensic?.graveyard_penalty ?? 0;
                        const credBreakdown = digitalTwinResult.legacy_forensic?.master_score_breakdown?.credibility;
                        ...
                        const order = ['FULFILLED', 'REVISED_UP', 'ON_TRACK', 'PARTIALLY_FULFILLED', 'REVISED_DOWN', 'MISSED'];
                          ...
                          .map(k => {
                            ...
                            return (
                              <span key={k} style={{
                                ...
                                background: colorMap[k], border: '1px solid var(--line)',
                                fontSize: '11px', fontWeight: 600, color: 'var(--text)',
                              }}>
                                <span style={{ fontWeight: 800 }}>{counts[k]}</span>
                                <span style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.3px', color: 'var(--muted)' }}>{k.replace(/_/g, ' ')}</span>
                              </span>
                            );
                          });

                        const graveyardBadge = (() => {
                        ...
                        return (
                          <div style={{
                            ...
                            {/* Secondary metrics row: trend, miss streak, narrative assessment */}
                            ...
                            {/* Graveyard rule alert (if any fired) */}
                            {graveyardBadge && <div style={{ marginTop: '12px' }}>{graveyardBadge}</div>}
