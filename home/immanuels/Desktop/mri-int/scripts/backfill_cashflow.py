...
def backfill_cashflow(symbol, cur):
    ...
    if "Operating Cash Flow" not in rows:
        logger.warning(f"  No Operating Cash Flow row for {symbol} (has: {rows[:5]})")
        return 0
    ...
    for year_col in cf.columns:
        ...
        ocf = sanitize_val(cf.loc["Operating Cash Flow", year_col]) if "Operating Cash Flow" in rows else None
...
def main():
    ...
    # Get all unique symbols from fundamental_financials
