...
- [x] **Full-Suite Tuple-Safety**: Hardened ALL 7 API routers (`auth`, `admin`, `signals`, `actions`, `portfolio`, `portfolio_review`, `watchlist`) to handle tuple cursor formats.
- [x] **Feature Recovery**: Resolved loading hangs in the Watchlist and Digital Twin (Holdings) views caused by backend index errors.
