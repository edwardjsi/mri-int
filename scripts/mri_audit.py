...
def audit():
    ...
    cur.execute("SELECT COUNT(*), MAX(date) FROM stock_scores")
    row = cur.fetchone()
    ...
    try:
        cur.execute("SELECT COUNT(*) FROM action_history WHERE action_type LIKE '%email%'")
        row = cur.fetchone()
