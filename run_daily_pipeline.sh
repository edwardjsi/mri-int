...
from engine_core.ingest_nifty500 import update_nifty500_universe
...
nifty_symbols = df['Symbol'].dropna().unique().tolist()
update_nifty500_universe(nifty_symbols)

symbols = set(nifty_symbols)
...
echo "[1.5/5] Ingesting Daily Market Cap..." | tee -a $LOG_FILE
python engine_core/ingest_market_cap.py 2>&1 | tee -a $LOG_FILE
...
echo "[5c] Sending Darvas Screener Alerts..." | tee -a $LOG_FILE
python engine_core/darvas_alerts.py 2>&1 | tee -a $LOG_FILE
