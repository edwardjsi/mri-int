...
Status: **DRAFT — awaiting user approval.** Full scope, time estimates, cost breakdown, and rollout plan in `docs/INITIATIVE_DATA_RICHNESS_2026-06-19.md`. Estimated wall time ~6-7 hrs, one-time LLM cost ~$5.00 (backfill + re-run). Rollback: all changes additive; default `'{}'::jsonb` keeps old paths working; revert PR if needed and the data stays in the JSONB column harmless. Four open questions in the FR doc with proposed defaults: JSONB schema (yes), all-at-once backfill (yes), graceful empty-state for un-backfillable stocks (yes), natural cache invalidation (yes).
